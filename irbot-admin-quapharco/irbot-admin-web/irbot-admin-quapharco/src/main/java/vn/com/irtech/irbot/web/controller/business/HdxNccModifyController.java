...
@Controller
@RequestMapping("/business/hdx-ncc-modify")
public class HdxNccModifyController extends BaseController {

	private String prefix = "business/hdx-ncc-modify";

	@Autowired
	private IHdxNccModifyService hdxNccModifyService;

	@Autowired
	private IHdxNccModifyDetailService hdxNccModifyDetailService;
	...
	public String processQuapharco(ModelMap mmap) {
		...
		return prefix + "/hdx-ncc-modify";
	}

	@PostMapping("/list")
	@ResponseBody
	public TableDataInfo list(HdxNcc hdxNcc) {
		startPage();
		List<HdxNcc> list = hdxNccModifyService.selectHdxNccModifyList(hdxNcc);
		return getDataTable(list);
	}

	@GetMapping("/detail/{id}")
	public String detail(@PathVariable("id") Long id, ModelMap mmap) {
		HdxNcc hdxNcc = hdxNccModifyService.selectHdxNccModifyById(id);
		mmap.put("hdxNccModify", hdxNcc);

		HdxNccDetail hdxNccDetail = new HdxNccDetail();
		hdxNccDetail.setHdxNccId(id);
		List<HdxNccDetail> details = hdxNccModifyDetailService.selectHdxNccModifyDetailList(hdxNccDetail);
	...
	public AjaxResult updateStatus(@Validated @RequestBody UpdateStatusReq request) {
		try {
			hdxNccModifyService.updateStatus(request.getIds(), request.getStatus());
	...
	public AjaxResult updateEInvoiceNo(@Validated @RequestBody UpdateEInvoiceNoReq request) {
		try {
			hdxNccModifyService.updateEInvoiceNo(request);
			AjaxResult ajaxResult = AjaxResult.success();
			return ajaxResult;
		} catch (Exception e) {
			logger.error(">>>>>> Error: " + e.getMessage());
