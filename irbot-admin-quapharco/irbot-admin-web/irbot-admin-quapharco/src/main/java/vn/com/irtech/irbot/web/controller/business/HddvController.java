...
@Controller
@RequestMapping("/business/hddv")
public class HddvController extends BaseController {

	private String prefix = "business/hddv";

	@Autowired
	private IHddvService hddvService;
	...
	@PostMapping("/list")
	@ResponseBody
	public Object list(Hddv hddv) throws Exception {
		try {
			return hddvService.selectHddvList(hddv);
		} catch (Exception e) {
	...
	public String detail(@PathVariable("id") String id, ModelMap mmap) {
		HddvMasterRes hddv = new HddvMasterRes();
		try {
			hddv = hddvService.selectHddvById(id);
	...
	public AjaxResult updateStatus(@Validated @RequestBody UpdateStatusReq request) {
		try {
			hddvService.updateStatus(request.getIds(), request.getStatus());
	...
	public AjaxResult updateMode(@Validated @RequestBody UpdateStatusReq request) {
		try {
			hddvService.updateMode(request.getIds());
			AjaxResult ajaxResult = AjaxResult.success();
			return ajaxResult;
		} catch (Exception e) {
			logger.error(">>>>>> Error: " + e.getMessage());
	...
	public AjaxResult updateEInvoiceNo(@Validated @RequestBody UpdateEInvoiceNoReq request) {
		try {
			hddvService.updateEInvoiceNo(request);
			AjaxResult ajaxResult = AjaxResult.success();
			return ajaxResult;
		} catch (Exception e) {
			logger.error(">>>>>> Error: " + e.getMessage());
	...
	public String checkModify(@PathVariable("fastId") String fastId, ModelMap mmap) {
		...
		try {
			result = hddvService.checkModify(fastId);
		} catch (Exception e) {
	...
	public String viewModifyDetail(@PathVariable("fastId") String fastId, @PathVariable("id") int id, ModelMap mmap) {
		try {
			TableDataInfo tableInfo = hddvService.checkModify(fastId);
	...
	public AjaxResult confirmModify(String fastIds) {
		try {
			hddvService.confirmModify(fastIds);
			AjaxResult ajaxResult = AjaxResult.success();
			return ajaxResult;
		} catch (Exception e) {
			logger.error(">>>>>> Error: " + e.getMessage());
