...
@Controller
@RequestMapping("/business/hdbh")
public class HdbhController extends BaseController {

	private String prefix = "business/hdbh";
	...
	@PostMapping("/list")
	@ResponseBody
	public Object list(Hdbh hdbh) throws Exception {
		try {
			return hdbhService.selectHdbhList(hdbh);
		} catch (Exception e) {
	...
	public String detail(@PathVariable("id") String id, ModelMap mmap) {
		HdbhMasterRes hdbh = new HdbhMasterRes();
		try {
			hdbh = hdbhService.selectHdbhById(id);
		} catch (Exception e) {
			logger.error(">>>>>> Error: " + e.getMessage());
		}
		mmap.put("hdbh", hdbh);
	...
	public AjaxResult updateStatus(@Validated @RequestBody UpdateStatusReq request) {
		try {
			hdbhService.updateStatus(request.getIds(), request.getStatus());
	...
	public AjaxResult updateMode(@Validated @RequestBody UpdateStatusReq request) {
		try {
			hdbhService.updateMode(request.getIds());
			AjaxResult ajaxResult = AjaxResult.success();
			return ajaxResult;
		} catch (Exception e) {
			logger.error(">>>>>> Error: " + e.getMessage());
	...
	public AjaxResult updateEInvoiceNo(@Validated @RequestBody UpdateEInvoiceNoReq request) {
		try {
			hdbhService.updateEInvoiceNo(request);
			AjaxResult ajaxResult = AjaxResult.success();
			return ajaxResult;
		} catch (Exception e) {
			logger.error(">>>>>> Error: " + e.getMessage());
	...
	public String checkModify(@PathVariable("fastId") String fastId, ModelMap mmap) {
		...
		try {
			result = hdbhService.checkModify(fastId);
		} catch (Exception e) {
	...
	public String viewModifyDetail(@PathVariable("fastId") String fastId, @PathVariable("id") int id, ModelMap mmap) {
		try {
			TableDataInfo tableInfo = hdbhService.checkModify(fastId);
	...
	public AjaxResult confirmModify(String fastIds) {
		try {
			hdbhService.confirmModify(fastIds);
			AjaxResult ajaxResult = AjaxResult.success();
			return ajaxResult;
		} catch (Exception e) {
			logger.error(">>>>>> Error: " + e.getMessage());
